<?php
    ...
    function logReserve($row, $place, $result) {
        if ($result) {
            echo nl2br("Ваше место забронировано! Ряд $row, место $place".PHP_EOL);
        } else {
            echo nl2br("Что-то пошло не так=( Бронь не удалась".PHP_EOL);
        }
    }
        ...
        for ($j = 0; $j < count($map); $j++) {
            ...
            for ($i = 0; $i < count($map[$j]); $i++) {
                ...
                if (!$map[$j][$i]) {
                    ...
                    $row = $j + 1;
                    $place = $i + 1;
                    $message .= nl2br("Ряд $row место $place ");
                } else {
                    $index = 0;
                    $message = "";
                }
